...
### Importations des séquences fast.q dans un objet path
...
### Séparation et tris des reverses et forwards
...
fnFs
```
### renommer les sequences dans sample.names 
...
### verifier la qualité des séquences
...
### Création d'un dossier filtered contenant les fichiers prêt pour filtration
...
paste0()-->faire un collage entre le vecteur sample.names et le suffixe "_F_filt.fastq.gz" (sequences forwards) et "R_filt.fastq.gz" (séquences reverses).
...
filtRs
```
### filtration et troncation

Ici, je viens stocker filtFs et filtRs, mes échantillons fnFs et fnRs une fois ceux-ci filtrés et tronqués par la fonction filterAndtrim(), puis je les mets dans un objet "out" comme ceci :
...
Lors du séquençage de l'ADN, il y a des erreurs aléatoires (mutations de nucléotides) qui peuvent se produire. Si on ne tient pas compte de ces erreurs, on risque de confondre des erreurs techniques avec de véritables variantes dans les séquences. DADA2 modélise ces erreurs pour corriger et identifier les séquences réelles présentes dans l'échantillon.

La fonction learnErrors ajuste un modèle statistique qui décrit la probabilité qu'un nucléotide soit incorrectement séquencé en fonction de la qualité de la base (souvent mesurée par un score de qualité Q, https://en.wikipedia.org/wiki/Phred_quality_score). Plus précisément, elle modélise la probabilité d'observer une séquence erronée étant donné les données de qualité associées à chaque base.
...
L'algorithme dada permet de differencier les differences de sequences dûent à des variations d'origine biologique ou à celles issue d'erreures de séquençage.
```{r}
...
La fonction mergePairs permet de réaliser l'alignement des séquences forward et reverse en fusionnant ces deux lectures pour obtenir la séquence complète cible. Une séquence fusionnée n'est conservée que si la région de chevauchement entre les deux lectures est parfaitement identique et comprend au moins 12 bases (ce seuil peut être ajusté). Dans cet exemple, le data.frame indique que le premier échantillon contient 127 séquences uniques, soit une variante de moins que lors de l'étape précédente, et affiche également l'abondance de chaque séquence dans l'échantillon
...
removeBimeraDenovo() :
Identifie et retire les séquences chimériques dans la table de séquences. Les chimères se forment lorsqu'un fragment d'ADN est mal répliqué pendant la PCR, donnant une séquence hybride entre deux séquences existantes, dûe à la partie conservée.
...
```
On a bien 20/20 en correspondance avec l'echantillon de reference mock, ce qui signifie qu'aucune erreure majeure dans le sequencage ni dans le pipeline dada2 n'est a declarer.
